...
def process_batch(
    ...
    for tools, conv in zip(batched_tools, batched_conv):
        ...
        batched_input_ids.append(input_ids+[0]*(max_input_length-len(input_ids)))
        batched_labels.append(labels+[0]*(max_input_length-len(labels)))
        print(len(batched_input_ids[-1]),len(batched_labels[-1]))
...
def load_tokenizer_and_model(
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_dir, trust_remote_code=True,model_max_length=max_input_length)
...
# @app.command()
def main(
        data_dir="datasets/chatglm3_6b",
        model_dir="/ssdata/application/zhuzh/workspace/download/models--THUDM--chatglm3-6b-1/snapshots/06c7c873c843814171c51330b69c2e2a68e05178",
        config_file="config/chatglm3_6b.yaml",
        deepspeed="/data/application/zhuzh/workspace/q3/peft/peft-main/config/qwen2/ds_config_zero3.json",
):

    ft_config = FinetuningConfig.from_file(config_file)
    tokenizer, model = load_tokenizer_and_model(model_dir, peft_config=ft_config.peft_config,max_input_length=ft_config.max_input_length)
